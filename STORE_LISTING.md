# 스토어 등록 문안 (Poing: Cloud Jump)

App Store Connect / Google Play 콘솔에 언어별로 붙여넣으세요.
글자 수 제한 — **App Store**: 앱 이름 30 · 부제 30 · 키워드 100 · 프로모션 170 · 설명 4000.
**Google Play**: 제목 30 · 간단한 설명 80 · 자세한 설명 4000.

앱 이름(브랜드)은 모든 언어 공통으로 **Poing: Cloud Jump** 를 권장합니다.

---

## 🇰🇷 한국어 (ko)

**부제 (30자):**
구름 위로 통통! 귀여운 고양이 점프

**프로모션 텍스트 (170자):**
꾹 눌렀다 떼서 구름 위로! 스윗스팟에서 떼면 퍼펙트 점프, 연속 착지로 콤보를 쌓으세요. 오브를 모아 전설·진화·트레이드오프 보상으로 나만의 빌드를 만들고 하늘 끝 우주까지!

**키워드 (쉼표 구분):**
고양이,점프,구름,아케이드,캐주얼,로그라이크,점수,오프라인,한손,미니게임,도약,하이스코어

**Google Play 간단한 설명 (80):**
꾹 눌렀다 떼면 점프! 구름을 밟고 하늘 끝까지 오르는 한 손 아케이드 점프 게임

**설명:**
구름 위를 통통 뛰어오르는 귀여운 고양이 아케이드 점프 게임!

꾹 눌러 힘을 모으고 떼서 도약하세요. 오래 누를수록 더 높이 뛰고, 벽에 부딪히면 방향이 바뀌며 지그재그로 올라갑니다. 게이지 스윗스팟에서 딱 맞춰 떼면 **퍼펙트 점프**, 안 떨어지고 연속으로 오르면 **콤보 배율**로 점수가 쑥쑥!

☁️ 두 가지 모드
• 일반 — 순수 점프 실력에 도전하는 하이스코어 모드
• 어드벤처 — 오브를 모아 강력한 보상 카드로 나만의 빌드를 완성

⭐ 로그라이크 깊이
• 룰을 바꾸는 전설 보상 (무한 자석 · 가시 파괴자 · 파동 마스터 …)
• 같은 보상을 쌓으면 강화형으로 진화
• 강력한 대신 대가가 있는 트레이드오프 카드
• 계열 세트 시너지와 숨은 시그니처 페어

낮 → 노을 → 밤 → 우주로 변하는 하늘을 오르며, 떠다니는 가시를 피하고 트램펄린·부스트 구름을 타고, 미끄러운 얼음 구름을 넘나드세요. 모은 코인으로 상점에서 영구 강화하고, 전 세계 최고 기록에 도전!

무료 · 한 손 · 세로 모드 · 가볍게 오프라인. 얼마나 높이 오를 수 있나요?

---

## 🇺🇸 English (en)

**Subtitle (30):**
Bounce the clouds, cute cat!

**Promotional Text (170):**
Press and release to leap up the clouds! Nail the sweet spot for a PERFECT jump, chain landings for combos, and collect orbs for legendary powers — all the way to space!

**Keywords:**
cat,jump,cloud,arcade,casual,roguelike,high score,offline,one hand,doodle,leap,minigame

**Google Play Short Description (80):**
Press and release to jump! Climb the clouds to space, one-handed.

**Description:**
Poing: Cloud Jump is a one-handed arcade jumper starring a cute cloud-hopping cat!

Press and hold to charge, then release to leap — hold longer to jump higher, and bounce off the walls to zigzag your way up. Let go right in the sweet spot for a **PERFECT** jump, and chain clean landings to build a score-boosting **combo**.

☁️ Two modes
• Classic — pure jumping skill, chase the high score
• Adventure — collect orbs, pick powerful reward cards, build your run

⭐ Roguelike depth
• Legendary powers that change the rules (infinite magnet, spike breaker, wave master…)
• Evolutions — stack a reward to upgrade it
• Trade-off cards — big power for a price
• Set synergies & hidden signature pairs

Climb through living skies — day, sunset, night, and outer space — dodging drifting spikes, riding trampolines and boosters, and hopping across slippery icy clouds. Spend coins in the shop for permanent upgrades, and race the world's best score.

Free, one hand, portrait, offline-friendly. How high can you climb?

---

## 🇯🇵 日本語 (ja)

**サブタイトル (30):**
雲の上でぴょん！かわいい猫ジャンプ

**プロモーションテキスト (170):**
長押しして離すと雲の上へ！スイートスポットで離せばパーフェクト、連続着地でコンボ。オーブを集めてレジェンド・進化・トレードオフの報酬で自分だけのビルドを作り、宇宙まで登ろう！

**キーワード:**
猫,ジャンプ,雲,アーケード,カジュアル,ローグライク,ハイスコア,オフライン,片手,ミニゲーム,飛躍

**Google Play 簡単な説明 (80):**
長押しして離すとジャンプ！雲を登って宇宙まで、片手で遊べるアーケードゲーム

**説明:**
雲の上をぴょんぴょん跳ねる、かわいい猫のアーケードジャンプゲーム！

長押しで力を溜めて、離してジャンプ。長く押すほど高く跳び、壁に当たると向きが変わってジグザグに登ります。ゲージのスイートスポットぴったりで離せば**パーフェクトジャンプ**、落ちずに連続で登れば**コンボ倍率**でスコアがぐんぐん！

☁️ 2つのモード
• ノーマル — 純粋なジャンプ技術に挑むハイスコアモード
• アドベンチャー — オーブを集め、強力な報酬カードで自分だけのビルドを完成

⭐ ローグライクの奥深さ
• ルールを変えるレジェンド報酬（無限磁石・トゲ破壊者・波動マスター…）
• 同じ報酬を重ねると強化形に進化
• 強力だが代償のあるトレードオフカード
• 系統セットシナジーと隠しシグネチャーペア

昼 → 夕焼け → 夜 → 宇宙へと変わる空を登り、漂うトゲを避け、トランポリンやブースト雲に乗り、滑る氷の雲を越えていこう。集めたコインでショップを永久強化し、世界最高スコアに挑戦！

無料・片手・縦向き・オフラインOK。どこまで高く登れる？

---

## 🇨🇳 中文（简体, zh）

**副标题 (30):**
云朵上弹跳！可爱猫咪跳跃

**推广文本 (170):**
长按后松开跃上云端！在甜点区松开触发完美跳跃，连续落地累积连击，收集宝珠获得传说、进化、权衡类奖励，一路冲向太空，打造专属构筑！

**关键词:**
猫,跳跃,云,街机,休闲,肉鸽,高分,离线,单手,小游戏,弹跳

**Google Play 简短说明 (80):**
长按后松开起跳！踩着云朵一路爬向太空的单手街机跳跃游戏

**描述:**
《Poing: Cloud Jump》是一款可爱猫咪在云朵间弹跳的单手街机跳跃游戏！

长按蓄力，松开跃起——按得越久跳得越高，撞到墙壁会转向，之字形向上攀爬。在能量槽的甜点区精准松开即可触发**完美跳跃**，不掉落连续攀爬则以**连击倍率**让分数飞涨！

☁️ 两种模式
• 经典 — 挑战纯粹跳跃技巧的高分模式
• 冒险 — 收集宝珠，用强力奖励卡打造专属构筑

⭐ 肉鸽深度
• 改变规则的传说奖励（无限磁铁·尖刺破坏者·波动大师……）
• 叠加同一奖励可进化为强化形态
• 强力却有代价的权衡卡牌
• 系别套装协同与隐藏签名组合

穿越昼→黄昏→夜→太空不断变化的天空，躲避漂浮尖刺，乘坐弹床与助推云，跨越湿滑的冰云。用金币在商店永久强化，冲击全球最高分！

免费·单手·竖屏·可离线畅玩。你能爬多高？

---

## 📝 참고
- **앱 이름**은 브랜드 일관성을 위해 4개 언어 모두 `Poing: Cloud Jump` 권장. (한/중/일 시장 검색 최적화를 원하면 현지어 병기 가능 — 예: `Poing: 클라우드 점프`)
- **스크린샷**은 언어별로 UI 언어를 바꿔 캡처하면 가장 좋습니다(앱 내 언어 버튼으로 전환 후 촬영).
- Google Play **간단한 설명(80자)** 은 각 언어 프로모션 텍스트를 80자로 줄여 사용하세요.
- 카테고리: 게임 > 아케이드 / 캐주얼. 연령 등급: 전체 이용가(광고 포함).
